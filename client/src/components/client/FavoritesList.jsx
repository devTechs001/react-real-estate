...
const Favorites = () => {
  ...
  return (
    <>
      <SEO title="My Favorites - HomeScape" />
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="px-4 py-2 border border-gray-200 rounded-xl focus:border-blue-500"
          >
          ...
          <div className="flex border border-gray-200 rounded-xl overflow-hidden">
            <button
              ...
              className={`p-2.5 ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
            ...
            <button
              ...
              className={`p-2.5 ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {favorites.map((property, index) => (
              ...
              >
                <div className={`relative ${viewMode === 'list' ? 'w-72 flex-shrink-0' : 'h-48'}`}>
                  ...
                  {property.status === 'sold' && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                      <span className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg">SOLD</span>
                    </div>
                  )}
                  {property.priceChange !== 0 && property.status !== 'sold' && (
                    <span className={`absolute top-3 left-3 px-2 py-1 rounded-full text-xs font-medium ${
                      property.priceChange < 0 
                        ? 'bg-green-100 text-green-700' 
                        : 'bg-red-100 text-red-700'
                    }`}>
                      {property.priceChange < 0 ? '↓' : '↑'} {formatPrice(Math.abs(property.priceChange))}
                    </span>
                  )}
                  <button
                ...
                <div className="p-4 flex-1">
                  ...
                  <p className="text-xl font-bold text-blue-600 mb-3">{formatPrice(property.price)}</p>
